...
def migrate_ldap_group_data_reverse(apps, schema_editor):
    ...
    base_dn_map = {
        g['MODEL_NAME'] : g['BASE_DN']
        for g in settings.JASMIN_SERVICES['LDAP_GROUPS']
    }
    Behaviour = apps.get_model("jasmin_services", "LdapGroupBehaviour")
    for behaviour in Behaviour.objects.all():
        behaviour.group_dn = "cn={},{}".format(behaviour.group_name,
                                               base_dn_map[behaviour.ldap_model])
        behaviour.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(migrate_ldap_group_data_forward,
                             migrate_ldap_group_data_reverse),
    ]
